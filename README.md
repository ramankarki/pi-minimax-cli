...
| Tool | Description |
| ---- | ----------- |
| `mmx_auth_login` | Authenticate via OAuth or API key |
| `mmx_auth_status` | Show current auth state and quota |
| `mmx_auth_refresh` | Manually refresh OAuth token |
| `mmx_auth_logout` | Revoke tokens and clear credentials |
...
| Tool | Description |
| ---- | ----------- |
| `mmx_config_show` | Display current config |
| `mmx_config_set` | Set a config value |
| `mmx_quota_show` | Show token plan usage |
| `mmx_config_export_schema` | Export tool schemas |
...
>
> Generated:
> *"Handcrafted sage green ceramic vase with a matte glaze finish.
> Perfect for minimalists seeking to add a touch of nature to their
> space. Each piece is unique with natural color variations."*
